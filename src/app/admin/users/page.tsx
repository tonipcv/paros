...
export default function AdminUsers() {
  ...
  const roleColor = (r: string) =>
    r === "SUPER_ADMIN" ? "text-rose-400" : r === "ADMIN" ? "text-amber-400" : "text-muted";
            ...
            {filtered.map((u) => (
              <tr key={u.id} className="hover:bg-bgHover">
                <td className="px-3 py-2.5">
                  <p className="text-[13px] font-medium text-primary truncate max-w-[180px]">{u.email}</p>
                  {u.name && <p className="text-[11px] text-muted truncate max-w-[180px]">{u.name}</p>}
                </td>
                <td className="px-3 py-2.5">
                  <span className={`text-[12px] font-semibold ${roleColor(u.role)}`}>{u.role}</span>
                </td>
                ...
                <td className="px-3 py-2.5 hidden sm:table-cell">
                  {u.emailVerified ? <span className="text-[11px] text-emerald-400">Yes</span> : <span className="text-[11px] text-muted">No</span>}
                </td>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    ...
                    {u.role !== "ADMIN" && u.role !== "SUPER_ADMIN" && (
                      <button onClick={() => updateRole(u.id, "ADMIN")}
                        className="rounded px-2 py-1 text-[10px] font-semibold bg-amber-500/15 text-amber-400 hover:bg-amber-500/25 transition">
                        Admin
                      </button>
                    )}
                    {u.role !== "USER" && (
                      <button onClick={() => updateRole(u.id, "USER")}
                        className="rounded px-2 py-1 text-[10px] font-semibold bg-gray-500/15 text-gray-400 hover:bg-gray-500/25 transition">
                        User
                      </button>
                    )}
                    {/* Credits */}
